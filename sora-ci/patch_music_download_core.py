...
def rep(path, old, new):
    ...
    if old not in text: raise SystemExit(f'missing block in {p}: {old[:80]!r}')
    p.write_text(text.replace(old, new, 1))

# Download model carries stable music identity/source metadata.
models = ROOT/'model/Models.kt'
rep(models, '''data class DownloadEntry(
    val id: String,
    val title: String,
...
repo = ROOT/'data/CoreRepository.kt'
rep(repo, '''                    sourceName = item.optString("sourceName"),
                    bytesDownloaded = item.optLong("bytesDownloaded"),''', '''                    sourceName = item.optString("sourceName"),
...
rep(repo, '''                put("sourceName", entry.sourceName)
                put("bytesDownloaded", entry.bytesDownloaded)''', '''                put("sourceName", entry.sourceName)
...
# Core-owned real downloader: resolves stream at download time, preserves headers,
# writes app-private Music bytes, persists progress, retries, deletes, and exposes local file lookup.
d = ROOT/'download'; d.mkdir(parents=True, exist_ok=True)
(d/'MusicDownloadManager.kt').write_text(r'''package com.night.sora.download
...
# Player local-first.
controller = ROOT/'playback/MusicPlaybackController.kt'
rep(controller, 'import java.util.concurrent.ConcurrentHashMap\n', 'import java.io.File\nimport java.util.concurrent.ConcurrentHashMap\n')
rep(controller, '    private var extensions: List<InstalledExtension> = emptyList()\n', '    private var extensions: List<InstalledExtension> = emptyList()\n    private var offlineResolver: ((ExtensionMediaSelection) -> String?)? = null\n')
rep(controller, '''    fun sessionPlayer(): Player = player
''', '''    fun setOfflineResolver(resolver: (ExtensionMediaSelection) -> String?) { offlineResolver = resolver }

    fun sessionPlayer(): Player = player
''')
rep(controller, '''    private fun resolveAndPlay(track: ExtensionMediaSelection) {
...
''', '''    private fun resolveAndPlay(track: ExtensionMediaSelection) {
        ...
        val extension = extensions.firstOrNull { it.packageName == track.extensionPackage }
''')
rep(controller, '''    private fun loadLyrics(extension: InstalledExtension, track: ExtensionMediaSelection, requestId: Long) {
