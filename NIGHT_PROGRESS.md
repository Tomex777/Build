...
## 2026-09-24 validation follow-up: Integrated Regression #432–#436

The branch is at `45c422c5728856fd752ac8f83f21d57fc92cbfed`. All Actions workflows for this source head completed before this progress update.

- Integrated Regression #432 proved that Android `MediaPlayer` rendered the synthetic MP4 directly into a `TextureView` (324,487 colored viewport pixels; ratio 0.9985), and Android `VideoView` rendered it through its `SurfaceView` (267,729 pixels; ratio 0.8239). Night's VLC view remained black (0 pixels). This isolates the emulator failure to the current libVLC path rather than the fixture or the TextureView type. The matching provider APKs passed all 31 device tests, and all 13 UI shards passed.
- Integrated Regression #433 tried VLC hardware decoding first on the virtual device. Its surface attached, but VLC never reached `playing`, playback stayed at 0 ms, and no Vout appeared. The platform baselines still rendered; video remained black through VLC. Build, 31 provider tests, all UI shards, ARM64 APK #114, and Groq Key Pool #392 passed.
- Integrated Regression #434 added a virtual-device Android `VideoView` fallback when VLC produced no output. The fallback prepared the media and the screenshot showed the colored fixture, but the interaction test failed because the fallback hid Night's custom controls.
- Integrated Regression #435 kept Night's controls and routed play, seek, speed, and time sampling to the fallback player. The video pixel assertion passed at 259,411 pixels (ratio 0.7983), and playback and seeking advanced; the test stopped at tap-to-hide because the native video view intercepted the tap.
- Integrated Regression #436 moved fallback surface taps to Compose. The full Android 16 video flow passed: Night's fallback rendered 263,339 colored viewport pixels (ratio 0.8104), hide/show controls and playback-speed checks passed, and the video editor preview rendered 228,424 colored pixels (ratio 0.7029). The installed AnimePahe test passed with a live descriptor of 5 tools, 6 message types, and 3,326 JSON characters. Provider instrumentation passed 31/31 tests, and all Android 16 UI shards passed. Night ARM64 APK #117 and Groq Key Pool #395 passed.
- The #432 Image Editor screenshot review showed a clear full-width preview, and its artifact summary confirmed the original image remained byte-for-byte unchanged. The #436 Image Editor UI suite also passed. This verifies emulator evidence only; comparison on the Samsung SM-A165F remains open.
