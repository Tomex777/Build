...
The #417 and #418 video regressions displayed a black preview. VLC parsed the fixture and started the H.264 decoder, but there was no Vout event; #418 again logged two successful `attachViews()` calls on the same player generation a millisecond apart. A local follow-up adds an atomic one-attach claim and records player/layout identity and surface dimensions; it needs Actions verification.
