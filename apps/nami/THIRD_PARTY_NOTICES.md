...
Nami's isolated anime compatibility module adapts the Aniyomi anime source contracts and host-side network integration. The current implementation targets a limited extensions-lib v16 surface and does not claim v17 binary compatibility. The Nami anime details presentation also uses Aniyomi's anime details experience as its behavior and layout reference. Relevant Apache-2.0 license text is retained in `licenses/ANIYOMI-APACHE-2.0.txt`; adapted API files carry local attribution headers.
